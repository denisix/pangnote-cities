...
function writeCity(city, pbf) {
  if (city.population > 1000) {
	  ...
	  pbf.writeStringField(3, cname[city.country])

	  const lat = Math.round(1e5 * city.lat)
	  const lng = Math.round(1e5 * city.lon)
	  pbf.writeSVarintField(4, lat - lastLat)
	  pbf.writeSVarintField(5, lng - lastLng)
	  lastLat = lat
	  lastLng = lng
  }
}
